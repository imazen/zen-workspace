...
## Synthetic-v2 corpus (the V_X base dataset)

The pre-V_X canonical training corpus — 218,089-pair synthetic
dataset assembled Feb–May 2026, used as the base for V0_18 onward
and still the largest CID22-clean synthetic source. **Now mirrored
to R2 and Tower (2026-05-22)** so future cloud feature-extraction
and remote training runs can pull the source images and pair tables
directly without depending on `/mnt/v/`.

| Field | Value |
|---|---|
| Status | **Mirrored 2026-05-22** — sources + pre-encoded distortions in `codec-corpus`, pair tables + score sidecars in `zentrain`, Tower NAS copy at `zensim-archive-2026-05-20` |
| Canonical pair table | `training_safe_synthetic.csv` (196,087 rows incl. header — **note: file row count differs from the 218,089-pair value documented in zensim/CLAUDE.md, latter includes a header miscount; treat 196,086 pairs as the load-bearing count**) |
| Canonical sha256 | `659982b3ce8d26184eca835a85f8d66c8550d945d659559c499b5670cd5d8589` (51,048,535 B) |
| Local source images | `/mnt/v/input/zensim/images/` (1,459,412 files, **451 GB**) |
| Local pair tables | `/mnt/v/output/zensim/synthetic-v2/` (222 files, 2.3 GB; excludes 2 circular symlinks at `synthetic-training` / `synthetic-training - Copy` — those point back to the same dir and are ignored by all mirror jobs) |
| R2 source images | `s3://codec-corpus/synthetic-v2/` (3,477 `<ref>/<codec_dir>/q<X>.png` subdirs) |
| R2 pair tables | `s3://zentrain/synthetic-v2/` (47 top-level files + `runs/` training-log subdir, 222 objects total) |
| Tower images mirror | `/mnt/tower/output/zensim-archive-2026-05-20/synthetic-v2-images/` |
| Tower tables mirror | `/mnt/tower/output/zensim-archive-2026-05-20/synthetic-v2-tables/` |
| Pre-encoded codecs | `mozjpeg-rs-420-e4` (43,673 cells), `zenjpeg-420-e2` (39,213), `zenavif-s5-e6` (34,001), `zenjpeg-420-xyb-e2` (28,182), `zenjxl-e7` (26,362), `zenwebp-default-m4` (24,655), `zenjpeg-420-e1` (122,117 e1-fill cells, abandoned mid-fill) |
| Score sidecars | `metric-ledger.jsonl` (174 MB, master scorer ledger), `features_gpu_ssim2.csv` (502 MB, derived feature snapshot — expensive but reproducible from sidecars) |
| Provenance | Created from `training_concordant.csv` minus all 49 CID22 validation image sources; 475 CID22-contaminated pairs removed. Zero CID22 validation leak verified. |
| Caveat | Source PNGs at `/mnt/v/input/zensim/sources/` (7.4 GB, 17,204 files) are referenced by `source_path` in pair tables but are NOT covered by this mirror — only the pre-encoded distortions under `images/` and the table CSVs ship. If a remote consumer needs the reference PNGs, mirror those separately. |

**Reading the synthetic-v2 pair table from R2:**
...
with fs.open("zentrain/synthetic-v2/training_safe_synthetic.csv") as f:
    df = pd.read_csv(f)
```

**Pulling a single image variant:**

```bash
s5cmd --endpoint-url "https://${R2_ACCOUNT_ID}.r2.cloudflarestorage.com" --profile r2 \
  cp 's3://codec-corpus/synthetic-v2/00b13be94a4867dd_1022x818/zenjpeg-420-e2/q50.png' .
```
...
> **Updated 2026-05-27 (cloud carve, spec Phase A/C/F):** `vastai-fleet`
> was renamed to the crate **`zenmetrics/crates/zen-cloud-vastai`** and the
> deployed binary is now the cloud-agnostic **`zen-sweep-worker`**
> (`crates/zen-sweep-worker`), which dispatches over a pluggable backend
> (`--backend vastai|salad|runpod|local`) on the `zen-cloud-core` trait
> layer. Provider crates: `zen-cloud-{core,s3,vastai,salad,runpod,local}`.
> See `~/work/zen/ARCHITECTURE.md` for the full fleet map. The old
> `crates/vastai-fleet` dir is deleted. References below kept for
> historical backfill provenance.

- Worker binary: **`zenmetrics/crates/zen-sweep-worker`** (was `crates/vastai-fleet`)
- CLI: `zen-sweep-worker worker --backend vastai --run-id <id> --chunks-r2 <uri>`
- Docker image: `ghcr.io/imazen/zen-metrics-sweep:v23` (historical); current vast `Dockerfile.sweep.v26`, salad `zen-metrics-sweep-salad:v3`
- Dockerfile: `zenmetrics/Dockerfile.sweep.v26` (current vast.ai image)
